    ...
    for resource in entry_list:
        if not resource.storage_object.deleted:
            ...
            data.append([resource.id, resource.get_absolute_url, resource, resource.storage_object.publication_status, \
                         getLRStats(resource.storage_object.identifier),
                         getUserCount(resource.storage_object.identifier, request.user.username), \
                         lastaccesstime])
                        ...
                        for _icomponent, _ifield, _irequired in instance_dbfields:
                            if not _icomponent in _classes:
                                ...
                                _add_usage_meta(usage_fields, component_name, \
                                                ...
                                                usagedata.get(
                                                    metaname.replace("Type_model", "").replace("String_model", ""),
                                                    None),
...
def _add_usage_meta(usage_fields, component_name, field, verbose_name, status, metaname_type, model_name, counters,
                    selected_filters, usage_filter):
    if counters is None:
        counters = [0, 0]
    _filters = []
...
def getstats(request):
    ...
    data['user'] = LRStats.objects.filter(lasttime__startswith=currdate).values('sessid').annotate(
        Count('sessid')).count()
    data['lrcount'] = resourceInfoType_model.objects.filter(
        storage_object__publication_status=PUBLISHED,
        storage_object__deleted=False).count()
    data['lrmastercount'] = StorageObject.objects.filter(copy_status=MASTER, publication_status=PUBLISHED,
                                                         deleted=False).count()
    ...
    extimes = QueryStats.objects.filter(lasttime__startswith=currdate).aggregate(Avg('exectime'), Max('exectime'),
                                                                                 Min('exectime'))

    qltavg = 0
    if (extimes["exectime__avg"]):
        qltavg = QueryStats.objects.filter(lasttime__startswith=currdate,
                                           exectime__lt=int(extimes["exectime__avg"])).count()
    else:
