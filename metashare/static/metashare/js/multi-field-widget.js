    ...
    if (e.val() != "") {
        $.ajax({
            ...
            success: function (result) {
                ...
                selElement.empty();
                selElement.append($("<option></option>").attr("value", "").text("--------"));
                for (var r in vars) {
                    selElement.append($("<option></option>").attr("value", vars[r]).text(vars[r]));
                }
...
function update_lang_variants_with_script(e) {
    ...
    if (e.val() != "") {
        $.ajax({
            ...
            data: {
                'script': e.val(),
                'lang': e.parent().parent().prev(".languageName").find("select option:selected").val()
            },
            success: function (result) {
                vars = result.split("//");
                selElement = variants.find("select:first");
...
function update_subdomains(e) {
    var root = $(e).closest(".dynamic-domaininfotype_model_set");
    var subdomain = root.find(".subdomain");
    ...
    if (e.val() != "-" && e.val() != "") {
        $.ajax({
            ...
            success: function (result) {
                vars = result.split("//");
                selElement = subdomain.find("select");
