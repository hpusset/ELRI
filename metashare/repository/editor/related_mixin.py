...
from metashare.repository.editor.lookups import ActorLookup, \
    ...
    LicenceInfoDummyLookup
from metashare.repository.models import actorInfoType_model, \
    documentationInfoType_model, organizationInfoType_model, \
    projectInfoType_model, personInfoType_model, \
...
class RelatedAdminMixin(object):
    ...
    def save_and_continue_in_popup(self, obj, request):
        ...
        return HttpResponse(
            '<script type="text/javascript">opener.saveAndContinuePopup(window, "%s", "%s", "%s");</script>' % \
    ...
    def edit_response_close_popup_magic(self, obj):
        ...
        return HttpResponse(
            '<html><body>'
            '<script type="text/javascript">opener.dismissEditRelatedPopup(window, "{}", "{}");</script>'
            .format(escape(pk_value), escapejs(obj))+
            '</body></html>')
